...
def get_skip_connection(_X, X):
    ...
    if( int(_X.get_shape()[2]) != int(X.get_shape()[2]) ):
       ...
       tmp = tf.keras.layers.Dense(units = out_dim, use_bias=False)(X)
       tmp = tf.cast(tmp, tf.float64)
       _X = tf.nn.relu(_X + tmp)
    else:
       _X = tf.nn.relu(_X + X) 
...
def encoder_gat_gate(X, A, num_layers):
    ...
    for i in range( len(hidden_dim)-1 ):
        ...
        for j in range( num_attn ):
            conv_weight.append( tf.compat.v1.get_variable('ecw'+str(i)+'_'+str(j), initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[i], hidden_dim[i+1]], dtype=tf.float64) )    
            conv_bias.append( tf.compat.v1.get_variable('ecb'+str(i)+'_'+str(j), initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[i+1]], dtype=tf.float64) )    
            attn_weight.append( tf.compat.v1.get_variable('eaw'+str(i)+'_'+str(j), initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[i+1], hidden_dim[i+1]], dtype=tf.float64) )    
...
def encoder_gcn_gate(X, A, num_layers):
    ...
    gates = []
    for i in range( len(hidden_dim)-1 ):
        conv_weight = tf.compat.v1.get_variable('ecw'+str(i), initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[i], hidden_dim[i+1]], dtype=tf.float64) 
        conv_bias = tf.compat.v1.get_variable('ecb'+str(i), initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[i+1]], dtype=tf.float64)
...
def readout_atomwise(X, latent_size):
    ...
    weight = {
        'mlp_f0': tf.compat.v1.get_variable("fw0", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[feature_size, hidden_dim[0]], dtype=tf.float64),
        'mlp_f1': tf.compat.v1.get_variable("fw1", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[0], hidden_dim[1]], dtype=tf.float64),
        'mlp_f2': tf.compat.v1.get_variable("fw2", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[1], hidden_dim[2]], dtype=tf.float64),
        'mlp_f3': tf.compat.v1.get_variable("fw3", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[2], hidden_dim[3]], dtype=tf.float64),
    }
    bias = {
        'mlp_f0': tf.compat.v1.get_variable("fb0", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[0]], dtype=tf.float64),
        'mlp_f1': tf.compat.v1.get_variable("fb1", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[1]], dtype=tf.float64),
        'mlp_f2': tf.compat.v1.get_variable("fb2", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[2]], dtype=tf.float64),
        'mlp_f3': tf.compat.v1.get_variable("fb3", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[3]], dtype=tf.float64),
    }
    ...
    _Y = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(Z, weight['mlp_f1'], bias['mlp_f1']))
    _Y = tf.nn.tanh(tf.compat.v1.nn.xw_plus_b(_Y, weight['mlp_f2'], bias['mlp_f2']))
    _Y = tf.compat.v1.nn.xw_plus_b(_Y, weight['mlp_f3'], bias['mlp_f3'])
...
def readout_graph_gather(X0, X1, latent_size):
    ...
    weight = {
        'mlp_f0i': tf.compat.v1.get_variable("fw0i", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[feature_size0+feature_size1, hidden_dim[0]], dtype=tf.float64),
        'mlp_f0j': tf.compat.v1.get_variable("fw0j", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[feature_size1, hidden_dim[0]], dtype=tf.float64),
        'mlp_f1': tf.compat.v1.get_variable("fw1", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[0], hidden_dim[1]], dtype=tf.float64),
        'mlp_f2': tf.compat.v1.get_variable("fw2", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[1], hidden_dim[2]], dtype=tf.float64),
        'mlp_f3': tf.compat.v1.get_variable("fw3", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[2], hidden_dim[3]], dtype=tf.float64),
    }
    bias = {
        'mlp_f0i': tf.compat.v1.get_variable("fb0i", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[0]], dtype=tf.float64),
        'mlp_f0j': tf.compat.v1.get_variable("fb0j", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[0]], dtype=tf.float64),
        'mlp_f1': tf.compat.v1.get_variable("fb1", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[1]], dtype=tf.float64),
        'mlp_f2': tf.compat.v1.get_variable("fb2", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[2]], dtype=tf.float64),
        'mlp_f3': tf.compat.v1.get_variable("fb3", initializer=tf.compat.v1.keras.initializers.glorot_normal(), shape=[hidden_dim[3]], dtype=tf.float64),
    }
